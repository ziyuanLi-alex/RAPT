...
* [ ] 从RS232 COM串口初始化RFID读取硬件。我的电脑上使用的是COM6，baud 115200，但是实际上应该写在简单的配置文件中。

* [ ] 读取RFID的Epc和RSSI，形成channel, RSSI, time的三维数据组。以相对时间为主，可选时间戳。

* [ ] RFID的读取频率应该尽可能高。开发文档中没有显式写明上报频次，这里需要调试，或者直接按照“尽力而为”逻辑。

* [ ] 用session的形式存储数据，可以手动给session添加“动作类型”这个tag。

* [ ] 使用可配置的滤波器进行初步预处理。优先使用中值滤波器，可选保留原读数。

* [ ] 由于Epc不直观，需要做RFID序号绑定。例如，将epc文件放置在读数器前面获取Epc，绑定对应标签为Tag 1。要有最简单的配置管理功能。
...
* [ ] 不做的功能一：除了6c epc之外任何类型的读数功能（GB，6B等）。
